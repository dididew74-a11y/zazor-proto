...
*Полный юридический текст лицензии содержится в файле `LICENSE` в корне репозитория.*
## 7. Безопасность
...
### v0.1 (Текущая версия) — Ядро протокола
- [x] Философский манифест и структура репозитория
- [x] Реализация `core/diplasty.py` (препроцессор с детекцией маркеров)
...
### v0.2 (Планируемая) — Расширение функционала
- [ ] Графический интерфейс (TUI на базе `textual` или `rich`)
- [ ] Поддержка Anthropic API и локальных моделей (Ollama, Llama.cpp)
- [ ] Модуль локального хранения резонансных следов (`trace`)
- [ ] Визуализация глитчей в реальном времени
- [ ] Принудительная задержка (`ui_delay.py`) в GUI

### v1.0 (Целевая) — Полноценный протокол
- [ ] Модуль семантического анализа на базе `sentence-transformers` для детекции категориального сдвига
- [ ] Полная поддержка всех основных API (OpenAI, Anthropic, Google, локальные модели)
- [ ] Документация для контрибьюторов и гайд по интеграции
- [ ] Сообщество «гильдии проводников» — peer-to-peer сеть супервизий
- [ ] Аудит безопасности и независимая проверка соответствия лицензии Anti-Enclosure
